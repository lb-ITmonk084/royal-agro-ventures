...
const WhyChooseUs = () => {
  ...
  const reasons = [
    { icon: <Tractor className="w-8 h-8" />, title: "Direct Farmer Sourcing", description: "We source directly from trusted farmers ensuring traceability and fair practices." },
    { icon: <ShieldCheck className="w-8 h-8" />, title: "Strict Hygiene & Safety", description: "Every product processed under rigorous hygiene and food safety standards." },
    { icon: <Thermometer className="w-8 h-8" />, title: "Quality-Controlled Dehydration", description: "Advanced dehydration process that retains natural color, flavor, and nutrients." },
    { icon: <TrendingUp className="w-8 h-8" />, title: "Consistent Supply", description: "Reliable supply capability to meet your business demands year-round." },
    { icon: <Package className="w-8 h-8" />, title: "Custom Packaging", description: "Flexible packaging options tailored to your specific requirements." },
    { icon: <Globe className="w-8 h-8" />, title: "Export-Oriented Approach", description: "Products processed to meet international quality and compliance standards." },
  ];
  ...
  return (
    <section className="py-24">
      <div className="container mx-auto px-4">
        ...
        <div ref={cardsRef} className={`grid sm:grid-cols-2 lg:grid-cols-3 gap-6 max-w-5xl mx-auto stagger-children ${cardsVisible ? "visible" : ""}`}>
        ...
        <div ref={ctaRef} className={`mt-16 bg-primary/5 rounded-3xl p-8 md:p-12 max-w-4xl mx-auto text-center reveal-scale ${ctaVisible ? "visible" : ""}`}>
