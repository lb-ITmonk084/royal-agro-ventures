import { 
  Sprout, Search, Droplets, Scissors, Thermometer, ClipboardCheck, Package, Truck 
} from "lucide-react";
...
const OurProcess = () => {
  ...
  const steps = [
    { icon: <Sprout className="w-6 h-6" />, title: "Direct Sourcing", description: "From trusted farmers", step: 1 },
    { icon: <Search className="w-6 h-6" />, title: "Inspection & Grading", description: "Raw material quality check", step: 2 },
    { icon: <Droplets className="w-6 h-6" />, title: "Cleaning & Washing", description: "Thorough sanitization", step: 3 },
    { icon: <Scissors className="w-6 h-6" />, title: "Preparation", description: "Cutting, slicing & sizing", step: 4 },
    { icon: <Thermometer className="w-6 h-6" />, title: "Dehydration", description: "Controlled drying process", step: 5 },
    { icon: <ClipboardCheck className="w-6 h-6" />, title: "Quality Checks", description: "Moisture & quality testing", step: 6 },
    { icon: <Package className="w-6 h-6" />, title: "Packaging", description: "Hygienic sealing", step: 7 },
    { icon: <Truck className="w-6 h-6" />, title: "Dispatch", description: "Export-ready storage", step: 8 },
  ];
  ...
  return (
    <section id="process" className="py-24">
      <div className="container mx-auto px-4">
        ...
        {/* Desktop Timeline */}
        <div ref={timelineRef} className={`hidden lg:block relative max-w-6xl mx-auto stagger-children ${timelineVisible ? "visible" : ""}`}>
        ...
        {/* Mobile/Tablet Grid */}
        <div className={`lg:hidden grid sm:grid-cols-2 gap-6 stagger-children ${timelineVisible ? "visible" : ""}`}>
          {steps.map((step, index) => (
            <div
              key={index}
